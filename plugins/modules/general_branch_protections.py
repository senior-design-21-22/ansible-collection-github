...
def run_module():
    module_args = dict(
        ...
        repository=dict(type='str', default='No Repo Provided.'),
    ...
    try:
        branch = g.get_repo(module.params['repository']).get_branch(module.params['branch'])
        if not branch.protected:
            initial = dict()
        else:
            url = branch.protection_url
            initial = requests.get(url, headers={'Authorization': 'Bearer {}'.format(module.params['access_token'])}).json()

    except Exception as e:
        module.exit_json(changed=False, err=e, failed=True)

    if module.params['state'] == 'present':
        try:
            branch = g.get_repo(module.params['repository']).get_branch(module.params['branch'])
            branch.edit_protection(strict=module.params['branch_protections']["strict"],
                                contexts=module.params['branch_protections']["contexts"],
                                enforce_admins=module.params['branch_protections']["enforce_admins"],
                                dismissal_users=module.params['branch_protections']["dismissal_users"],
                                dismissal_teams=module.params['branch_protections']["dismissal_teams"],
                                dismiss_stale_reviews=module.params['branch_protections']["dismiss_stale_reviews"],
                                require_code_owner_reviews=module.params['branch_protections']["require_code_owner_reviews"],
                                required_approving_review_count=module.params['branch_protections']["required_approving_review_count"],
                                user_push_restrictions=module.params['branch_protections']["user_push_restrictions"],
                                team_push_restrictions=module.params['branch_protections']["team_push_restrictions"])
        except Exception as e:
            module.exit_json(changed=False, err=e, failed=True)
    ...
    try:
        ...
        if not branch.protected:
            ...
            url = branch.protection_url
            output = requests.get(url, headers={'Authorization': 'Bearer {}'.format(module.params['access_token'])}).json()
    except Exception as e:
        module.exit_json(changed=False, err=e, failed=True)
